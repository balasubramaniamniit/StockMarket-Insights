...
class nlpinsights:
    ...
    def get_topwords_key(self, key, top_n):
        if self.column_name:
            df = self.full_df.copy()
        lis = []
        for word in " ".join(df[self.column_name]).split(' '):
            if key in word:
                lis.append(word)
        return pd.Series(lis).value_counts()[:top_n]
    ...
    def get_namedentities(self):
        ...
        namedent_agg = namedentdf.groupby(['Label', 'Text']).size().reset_index()
        namedent_agg.columns = ['Label', 'Text', 'count']
        namedent_agg_sorted = namedent_agg.groupby(['Label']).apply(
            lambda x: x.sort_values(['count'], ascending=False)).reset_index(drop=True)
        namedent_agg_top5_df = namedent_agg_sorted.groupby(['Label']).head(5)
        return namedent_agg_top5_df
    ...
    def get_full_nlpinsights(self):
        ...
        nlpinsightsdic['sentiments_df'] = self.get_sentiment_df().to_dict(orient='records')
    ...
    def treemap_chart(self, tree_data, path):
        """
    ...
    def visualize_namedentities(self, save_fig=False, spacy_fig=False):
        ...
        namedentitytree_fig = self.treemap_chart(namedentdf,path=['Label', 'Text'])
    ...
    def get_sentiment_df(self):
        ...
        """
        if self.column_name:
            sentiment_df = self.full_df.copy()
        else:
            sents = sent_tokenize(self.data)
            sentiment_df = pd.DataFrame(sents, columns=['text'])

        sentiment_df['sentiment_score'] = sentiment_df['text'].apply(
            lambda x: self.get_sentiments(x)['compound'])
        sentiment_df['sentiment'] = sentiment_df['text'].apply(lambda x: self.get_sentiment_labels(x))
